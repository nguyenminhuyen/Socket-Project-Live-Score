...
class Server:
    ...
    def connect(self):
        ...
        try:
            while True:
                ...
                self.logger.log(logging.INFO,"Connected by " + str(addr))
                print('Connected by', addr)
                start_new_thread(self.threadClient, (client, ))
                self.threadCount += 1
                self.logger.log(logging.INFO,"Thread num: " + self.threadCount)
    ...
    def login(self,sock):
        ...
        sub = json.loads(sub)
        self.logger.log(logging.INFO,"Usr:" + str(sub['usr']) + " - Pass:" + str(sub['psw']))   
        if (sub['usr'] == ADMIN_USR and sub['psw'] == ADMIN_PSW):
            print(2)
            self.sendData(sock, '2') # Admin đăng nhập thành công
            self.logger.log(logging.INFO,"Đăng nhập admin thành công")
            return 2    
        ...
        for acc in accs["account"]:
            ...
            if u == sub['usr']:
                if p == sub['psw']:
                    ...
                    self.logger.log(logging.INFO,"Đăng nhập thành công")
                    return 1
                else:
    ...
    def regist(self, sock):
        ...
        sub = self.receive(sock)
        sub = json.loads(sub)
        self.logger.log(logging.INFO,"Usr:" + sub['usr'] + " - Pass:" + sub['psw']) 
        fd = open("account.json", "r+")
        userExist = False
        accs = json.loads(fd.read())
        for acc in accs["account"]:
        ...
        if not userExist:
            ...
            self.logger.log(logging.INFO,"Đăng ký thành công")
        fd.close()
        return True

    def matchDetail(self,sock):
        id = self.receive(sock)
        self.logger.log(logging.INFO,"ID trận đấu cần mở: " + id)
        ...
        if (pos1 != -1 and pos2 != -1):
            ...
            self.logger.log(logging.INFO,"Mở match detail thành công")
        else:
    ...
    def listMatch(self,sock):
        ...
        self.logger.log(logging.INFO,"Mở danh sách trận đấu thành công")
        #self.sendData(sock, '0')
        return False
    ...
    def addMatch(self,sock):
        ...
        self.logger.log(logging.INFO,str(data_add))
        f = open("listmatch.json", "r+")
        data = json.load(f)
        ...
        self.logger.log(logging.INFO,"Thêm trận đấu thành công")
        f.close()
        return True
    ...
    def updMatch(self, sock):   
        ...
        if (pos != -1):
            ...
            self.logger.log(logging.INFO,"Cập nhật trận đấu (score, time) thành công")
            f.close()
            return True
    ...
    def upEvent(self, sock):   
        ...
        if (pos != -1):
            ...
            self.logger.log(logging.INFO,"Thêm sự kiện thành công")
            print(1)
            f.close()
            return True
