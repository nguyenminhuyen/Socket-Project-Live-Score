...
class ConsoleUi:
    ...
    def __init__(self, frame):
        ...
        self.scrolled_text.tag_config('DATA', foreground='gray')
        self.scrolled_text.tag_config('REQUEST', foreground='blue')
        self.scrolled_text.tag_config('ERROR', foreground='red')
        self.scrolled_text.tag_config('RESULT', foreground='orange')
...
class ThirdUi:
    ...
    def __init__(self, frame):
        self.frame = frame
        ttk.Label(self.frame, text = "LIVE SCORE", font = ('Times', 30, 'bold')).grid(column=0, row=1, sticky=W)
        ttk.Label(self.frame, text = "Server", font = ('Times', 20)).grid(column=0, row=4, sticky=W) 
...
def main():
    ...
    root = Tk()
    root.resizable(0,0)
    ttk.Label(root, text = "LIVE SCORE", font = ('Times', 30, 'bold')).grid(column=0, row=0, sticky=W)
    ttk.Label(root, text = "Server", font = ('Times', 20)).grid(column=0, row=4, sticky=W) 
    ttk.Label(root, text = "Nhập số client cho phép kết nối: ").grid(column=0, row=6, sticky=W)
    nVar = StringVar()
    nEntry = ttk.Entry(root,textvariable= nVar, width = 50).grid(column=0, row=8, sticky=W)
    nFunc = partial(submitNumThread,root, nVar)
    ttk.Button(root, text = "Submit", command = nFunc).grid(column=0, row=10, sticky=W)
    root.mainloop()
